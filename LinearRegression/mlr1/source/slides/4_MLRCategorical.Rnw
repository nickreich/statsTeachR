...
\bi
	\myitem Let $x$ be a categorical variable with $k$ levels (e.g. with $k=3$ ``low'', ``med'', ``hi'').
        \myitem Choose one group as the baseline (e.g. ``low'')
	\myitem Create $(k-1)$ binary terms to include in the model:
\begin{eqnarray*}
        x_{1,i} & = & \mathbbm{1} (x_i=\mbox{``med''}) \\
        x_{2,i} & = & \mathbbm{1} (x_i=\mbox{``hi''}) 
\end{eqnarray*}
